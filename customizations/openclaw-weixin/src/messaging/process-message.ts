...
export async function processOneMessage(
  ...
  if (agentSwitchResult.handled) {
    logger.info(`[weixin] Agent switch command handled, skipping AI pipeline`);
    return;
  }
  ...
  let route = deps.channelRuntime.routing.resolveAgentRoute({
    cfg: deps.config,
    channel: "openclaw-weixin",
    accountId: deps.accountId,
    peer: { kind: "direct", id: ctx.To },
  });
  route = applyAgentOverrideToRoute(route, {
    cfg: deps.config,
    accountId: deps.accountId,
    peerId: ctx.To,
    channel: "openclaw-weixin",
